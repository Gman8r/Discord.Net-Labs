...
namespace Discord.Rest
{
    internal static class MessageHelper
    {
        ...
        /// <exception cref="InvalidOperationException">Only the author of a message may modify the message.</exception>
        /// <exception cref="ArgumentOutOfRangeException">Message content is too long, length must be less or equal to <see cref="DiscordConfig.MaxMessageSize"/>.</exception>
        public static async Task<Model> ModifyWithFileAsync(IMessage msg, BaseDiscordClient client, Stream stream, string filename, Action<MessageProperties> func,
            RequestOptions options)
        {
            var args = new MessageProperties();
            func(args);
            ...
            var apiArgs = new ModifyMessageWithFileParams(stream)
            {
                Filename = filename,
                Content = args.Content,
                Embeds = apiEmbeds?.ToArray() ?? Optional<API.Embed[]>.Unspecified,
                MessageComponent = args.Components.IsSpecified ? args.Components.Value?.Components.Select(x => new API.ActionRowComponent(x)).ToArray() ?? new API.ActionRowComponent[0] : Optional<API.ActionRowComponent[]>.Unspecified,
                Flags = args.Flags,
                AllowedMentions = args.AllowedMentions.IsSpecified ? args.AllowedMentions.Value.ToModel() : Optional<API.AllowedMentions>.Unspecified,
            };
            return await client.ApiClient.ModifyMesageWithFileAsync(msg.Channel.Id, msg.Id, apiArgs, options).ConfigureAwait(false);
        }
